    ...
    [
        [
            InlineKeyboardButton(
                text="اشترك هنا", url="https://t.me/deiiiiiiiiiiiiiiiiiiiiiiiiii"
            ),                        
        ],        
    ]
)
...
async def check_is_joined(message):    
    try:
        ...
        status = await app.get_chat_member("deiiiiiiiiiiiiiiiiiiiiiiiiii", userid)
        return True
    except Exception:
        await message.reply_text("*انت لست مشترك في قناة البوت @deiiiiiiiiiiiiiiiiiiiiiiiiii ** \n**انضم لتستطيع تشغيل الاغاني**",reply_markup=force_btn,parse_mode="markdown",disable_web_page_preview=False)
        return False
